	...
	if (panelWork) {
		...
		if (dist > 10) {
			...
			if (abs(nowPos.x - panelPos.x) < dist *.3827f) {
				if (nowPos.y < panelPos.y)
					if (ctrling)
						putString("文件开头", panelPos.x - 30, panelPos.y - 8);
					else
						putString("上加一行", panelPos.x - 30, panelPos.y - 8);
				if (nowPos.y > panelPos.y)
					if (ctrling)
						putString("文件结尾", panelPos.x - 30, panelPos.y - 8);
					else
						putString("下加一行", panelPos.x - 30, panelPos.y - 8);
			}
			else if (abs(nowPos.y - panelPos.y) < dist *.3827f) {
				if (nowPos.x < panelPos.x)
					if (ctrling)
						putString("整行左侧", panelPos.x - 30, panelPos.y - 8);
					else
						putString("标识左侧", panelPos.x - 30, panelPos.y - 8);
				if (nowPos.x > panelPos.x)
					if (ctrling)
						putString("整行右侧", panelPos.x - 30, panelPos.y - 8);
					else
						putString("标识右侧", panelPos.x - 30, panelPos.y - 8);
			}
			else if (nowPos.x < panelPos.x && nowPos.y < panelPos.y)
				if (ctrling)
					putString("编写脚本", panelPos.x - 30, panelPos.y - 8);
				else
					putString("撤销操作", panelPos.x - 30, panelPos.y - 8);
			else if (nowPos.x > panelPos.x && nowPos.y < panelPos.y)
				if (ctrling)
					putString("版本更新", panelPos.x - 30, panelPos.y - 8);
				else
					putString("重做操作", panelPos.x - 30, panelPos.y - 8);
			else if (nowPos.x < panelPos.x && nowPos.y > panelPos.y)
				if (ctrling)
					putString("左大括号", panelPos.x - 30, panelPos.y - 8);
				else
					putString("左中括号", panelPos.x - 30, panelPos.y - 8);
			else if (nowPos.x > panelPos.x && nowPos.y > panelPos.y)
				if (ctrling)
					putString("右大括号", panelPos.x - 30, panelPos.y - 8);
				else
					putString("右中括号", panelPos.x - 30, panelPos.y - 8);
		}
	}
}
