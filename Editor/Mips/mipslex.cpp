...
vector<mipsTokenPrim> MipsLex::parse() {
	...
	for (int i = 0; i < len; i++) {
		mipsTokenPrim node;
		if (content[i] == '\n')tmpLine++;
		else if (content[i] == ' ' || content[i] == '\t')continue;
		else if (content[i] == '-' ||(content[i] >= '0' && content[i] <= '9')) {
			node.type = MIPS_TT_DATA;
			node.start = i;
			node.line = tmpLine + 1;

			if (content[i] == '0' && (content[i + 1] == 'x' || content[i + 1] == 'X')) {
				i += 2;
				node.value = float(strtol(content.data() + i, NULL, 16));
			...
			while (content[i] >= 'A'&&content[i] <= 'Z' || content[i] >= 'a'&&content[i] <= 'z' ||
				content[i] >= '0'&&content[i] <= '9' || content[i] == '_' || content[i] == '.') {
				str[j++] = content[i++];
				if (j == 31) {
					str[j] = '\0';
			...
			case '$':
				...
				if (content[i] == 'z') {
					...
					else if (content[i + 1] >= '8' && content[i + 1] <= '9')node.id = content[i + 1] - '0' + 16;
					else error("register", MIPS_LE_ILLEGAL);
					i++;
				}
				else if (content[i] == 's') {
