		...
		if (content[proc].type == SGS_TT_SYS&&content[proc].id == SGS_ID_USE) {
			...
			if (content[proc].type == SGS_TT_SYS && content[proc].id == SGS_ID_ELSE) {
				...
				while (content[proc].type == SGS_TT_SYS && content[proc].id == SGS_ID_IF) {
					...
					rtVal seq = rtVal();
					seq.op = VO_IF;
					seq.left = parseExpression();
					seq.right = new varNode(_VT_FUNCTION, "");
					if (content[proc].type != SGS_TT_SYS || content[proc].id != SGS_ID_THEN)
						error("If(then)", SGS_SE_INCOMPLETE);
					proc++;
					varNode *seqacc = new varNode(_VT_FUNCTION, "");
					seqacc->block = parseBlock();
					seq.right->left = seqacc;
					seq.right->right = new varNode(_VT_FUNCTION, "");
					reject->block = new stateSeq();
					reject->block->act = seq;
					reject->block->next = NULL;
					reject = reject->block->act.right->right;
				...
				if (last)reject->block = parseBlock();
			}
	...
	while (proc < content.size()) {
		if (content[proc].type == SGS_TT_SYS && content[proc].id != SGS_ID_RESULT &&
			...
			if (content[proc].s != NULL) {
				tmp->t = _VT_STRING;
				tmp->val = malloc(strlen(content[proc].s) + 1);
