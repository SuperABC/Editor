...
varNode SgsMachine::exp(varNode *e) {
	...
	if (e->t == _VT_FUNCTION) {
		...
		case SGS_OP_MOD:
			...
			break;
		case SGS_OP_EQUAL: {
			float op1 = 0.f, op2 = 0.f;
			if (l.t == _VT_INTEGER)op2 = (float)*(int *)(l.val);
			if (l.t == _VT_FLOAT)op2 = *(float *)(l.val);
			if (r.t == _VT_INTEGER)op1 = (float)*(int *)(r.val);
			if (r.t == _VT_FLOAT)op1 = *(float *)(r.val);
			ret.t = _VT_BOOL;
			ret.val = new bool(op1 == op2);
			break;
		}
		case SGS_OP_GREATER: {
			float op1 = 0.f, op2 = 0.f;
		...
		case SGS_OP_ANDAND: {
			...
			if (l.t == _VT_INTEGER)op2 = *(int *)(l.val);
			if (l.t == _VT_BOOL)op2 = *(bool *)(l.val);
			if (l.t == _VT_FLOAT)op2 = *(float *)(l.val);
			if (r.t == _VT_INTEGER)op1 = *(int *)(r.val);
			if (r.t == _VT_BOOL)op1 = *(bool *)(r.val);
			if (r.t == _VT_FLOAT)op1 = *(float *)(r.val);
			ret.t = _VT_BOOL;
			ret.val = new bool(op1 && op2);
			break;
